...
public:
	...
	DLList operator+(const DLList& ref)
	{
		...
		node* temp_ref = ref.head;

		while (temp_this != NULL && temp_ref != NULL)
		{
			if (temp_this->getData() < temp_ref->getData())
			{
				newlist.append(temp_this->getData());
				temp_this = temp_this->next;
			}
			else
			{
				newlist.append(temp_ref->getData());
